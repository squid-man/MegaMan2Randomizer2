...
namespace MM2Randomizer.Randomizers
{
    public class RBossAI : IRandomizer
    {
        ...
        protected void ChangeHeat(Patch in_Patch, ISeed in_Seed)
        {
            ...
            in_Patch.Add(0x02C207, in_Seed.GetNextUInt8(3, 9), "Heatman Projectile 1 Y-Distance");

            //0x02C208 default 05, good from 04 - 07
            in_Patch.Add(0x02C208, in_Seed.GetNextUInt8(4, 8), "Heatman Projectile 2 Y-Distance");

            //0x02C209 default 03, good from 03 - 05
            in_Patch.Add(0x02C209, in_Seed.GetNextUInt8(3, 6), "Heatman Projectile 3 Y-Distance");
            ...
            in_Patch.Add(0x02C20A, in_Seed.GetNextUInt8(48, 129), "Heatman Projectile 1 X-Distance");

            //0x02C20B - 2nd value should be 46 to hit megaman, 0r, from 34 to 64
            in_Patch.Add(0x02C20B, in_Seed.GetNextUInt8(34, 65), "Heatman Projectile 2 X-Distance");

            //0x02C20C - 3rd value should be 23 to hit megaman, or, from 16 to 48
            in_Patch.Add(0x02C20C, in_Seed.GetNextUInt8(16, 49), "Heatman Projectile 3 X-Distance");
            ...
            Byte delay = in_Seed.GetNextUInt8(10, 41);
            ...
            in_Patch.Add(0x02C253, in_Seed.GetNextUInt8(2, 6), "Heatman Charge Velocity");
        }
        ...
        protected void ChangeAir(Patch in_Patch, ISeed in_Seed)
        {
            ...
            {
                in_Patch.Add(0x02C393 + i, in_Seed.GetNextUInt8(256), String.Format("Airman Tornado {0} Y-Vel Frac", i));
            }
            ...
            for (Int32 i = 0; i < TORNADO_TABLE_LENGTH; i++)
            {
                ...
                if (in_Seed.GetNextDouble() > 0.9)
                {
                    velocityY = 0x04;
                }
                else
                {
                    velocityY = in_Seed.GetNextElement(VELOCITY_Y_TABLE);
                }
            ...
            for (Int32 i = 0; i < TORNADO_TABLE_LENGTH; i++)
            {
                in_Patch.Add(0x02C3CF + i, in_Seed.GetNextUInt8(256), String.Format("Airman Tornado {0} X-Vel Frac", i));
            }
            ...
            for (Int32 i = 0; i < TORNADO_TABLE_LENGTH; i++)
            {
                ...
                Double chance = in_Seed.GetNextDouble();
                ...
                {
                    velocityX = in_Seed.GetNextElement(VELOCITY_X_TABLE);
                }
            ...
            for (Int32 i = 0; i < TORNADO_TABLE_LENGTH; i++)
            {
                in_Patch.Add(0x02C40B + i, in_Seed.GetNextUInt8(5, 42), String.Format("Airman Tornado {0} Delay Time", i));
            }

            // 0x02C30C - Num patterns before jumping 0x03 (do 1-4)
            in_Patch.Add(0x02C30C, in_Seed.GetNextUInt8(1, 5), "Airman Patterns Before Jump");
            ...
            Byte rSum = in_Seed.GetNextUInt8(2, 4);
            Byte jump1x = in_Seed.GetNextUInt8(rSum + 1);
            ...
            in_Patch.Add(0x02C4DD, in_Seed.GetNextUInt8(241), "Airman Y-Velocity Fraction Jump 1");
            in_Patch.Add(0x02C4DE, in_Seed.GetNextUInt8(241), "Airman Y-Velocity Fraction Jump 2");
            in_Patch.Add(0x02C4E3, in_Seed.GetNextUInt8(256), "Airman X-Velocity Fraction Jump 1");
            in_Patch.Add(0x02C4E4, in_Seed.GetNextUInt8(256), "Airman X-Velocity Fraction Jump 2");
        }
        ...
        private Byte AirmanGetJumpYVelocity(Int32 xVelInt, ISeed in_Seed)
        {
            ...
            return in_Seed.GetNextUInt8(jumpYMin, jumpYMax + 1);
        }
        ...
        protected void ChangeWood(Patch in_Patch, ISeed in_Seed)
        {
            ...
            in_Patch.Add(0x02C537, in_Seed.GetNextUInt8(6, 33), "Woodman Leaf Spacing Delay");

            //0x02C5DD - Jump height, 4. Do 3 to 7.
            in_Patch.Add(0x02C5DD, in_Seed.GetNextUInt8(3, 8), "Woodman Jump Y-Velocity");

            //0x02C5E2 - Jump distance, 1. Do 1 to 4.
            in_Patch.Add(0x02C5E2, in_Seed.GetNextUInt8(1, 5), "Woodman Jump X-Velocity");

            //0x02C5A9 - Shield launch speed, 4. Do 1 to 8.
            in_Patch.Add(0x02C5A9, in_Seed.GetNextUInt8(1, 9), "Woodman Shield Launch X-Velocity");

            //0x02C553 - Number of falling leaves, 0x03. Do 0x02 20% of the time.
            if (in_Seed.GetNextDouble() > 0.8)
            {
            ...
            in_Patch.Add(0x02C576, in_Seed.GetNextElement(VELOCITY_X_TABLE), "Woodman Falling Leaf X-Velocity");
        ...
        protected void ChangeBubble(Patch in_Patch, ISeed in_Seed)
        {
            ...
            in_Patch.Add(0x02C707, in_Seed.GetNextElement(BUBBLEMAN_HEIGHT_APEX_TABLE), "Bubbleman Y Max Height");

            //0x02C70B - Falling speed integer, 0xFF.
            Byte[] BUBBLEMAN_FALL_SPEED_TABLE = { 254, 255, 255, 255, 255 };
            in_Patch.Add(0x02C70B, in_Seed.GetNextElement(BUBBLEMAN_FALL_SPEED_TABLE), "Bubbleman Y-Velocity Falling");

            //0x02C710 - Landing x-tracking speed, integer, 0x00.
            Byte[] BUBBLEMAN_HORIZONTAL_TRACKING_SPEED_TABLE = { 0, 0, 0, 0, 0, 1 };
            in_Patch.Add(0x02C710, in_Seed.GetNextElement(BUBBLEMAN_HORIZONTAL_TRACKING_SPEED_TABLE), "Bubbleman X-Velocity Falling");

            //0x02C6D3 - Rising speed integer, 0x01.
            Byte[] BUBBLEMAN_RISING_SPEED_TABLE = { 1, 1, 2, 2, 3 };
            in_Patch.Add(0x02C6D3, in_Seed.GetNextElement(BUBBLEMAN_RISING_SPEED_TABLE), "Bubbleman Y-Velocity Rising");

            //0x02C745 - Delay between water gun shots, 0x12
            Byte[] BUBBLEMAN_ARM_CANNON_SHOT_DELAY_TABLE = { 4, 8, 12, 16, 18, 20, 24, 28 };
            in_Patch.Add(0x02C745, in_Seed.GetNextElement(BUBBLEMAN_ARM_CANNON_SHOT_DELAY_TABLE), "Bubbleman Water Gun Cooldown");
            ...
            in_Patch.Add(0x03DA25, in_Seed.GetNextElement(BUBBLEMAN_BUBBLE_LEAD_SHOT_X_VELOCITY_TABLE), "Bubbleman X-Vel Bubble, Int");

            //0x03DA26 - X-Vel bubble shot, Frac 0x00
            Byte[] BUBBLEMAN_BUBBLE_LEAD_SHOT_X_VELOCITY_FRACTION_TABLE = { 128, 192, 255 };
            in_Patch.Add(0x03DA26, in_Seed.GetNextElement(BUBBLEMAN_BUBBLE_LEAD_SHOT_X_VELOCITY_FRACTION_TABLE), "Bubbleman X-Vel Bubble, Frac");

            //0x03DA4D - Y-Vel bubble shot initial, Int (0x03)
            Byte[] BUBBLEMAN_BUBBLE_LEAD_SHOT_Y_VELOCITY_TABLE = { 2, 3, 4, 5 };
            in_Patch.Add(0x03DA4D, in_Seed.GetNextElement(BUBBLEMAN_BUBBLE_LEAD_SHOT_Y_VELOCITY_TABLE), "Bubbleman Y-Vel Bubble Initial, Int");

            //0x03DA4E - Y-Vel bubble shot initial, Frac (0x76)
            Byte[] BUBBLEMAN_BUBBLE_LEAD_SHOT_Y_VELOCITY_FRACTION_TABLE = { 0, 64, 128, 192 };
            in_Patch.Add(0x03DA4E, in_Seed.GetNextElement(BUBBLEMAN_BUBBLE_LEAD_SHOT_Y_VELOCITY_FRACTION_TABLE), "Bubbleman Y-Vel Bubble Initial, Frac");

            //0x03B747 - Y-Vel bubble shot bounce, Int (0x03)
            Byte[] BUBBLEMAN_BUBBLE_LEAD_BOUNCE_Y_VELOCITY_TABLE = { 2, 3, 4, 5 };
            in_Patch.Add(0x03B747, in_Seed.GetNextElement(BUBBLEMAN_BUBBLE_LEAD_BOUNCE_Y_VELOCITY_TABLE), "Bubbleman Y-Vel Bubble Bounce, Int");

            //0x03B74C - Y-Vel bubble shot bounce, Frac (0x76)
            Byte[] BUBBLEMAN_BUBBLE_LEAD_BOUNCE_Y_VELOCITY_FRACTION_TABLE = { 0, 64, 128, 192 };
            in_Patch.Add(0x03B74C, in_Seed.GetNextElement(BUBBLEMAN_BUBBLE_LEAD_BOUNCE_Y_VELOCITY_FRACTION_TABLE), "Bubbleman Y-Vel Bubble Bounce, Frac");
        }
        ...
        protected void ChangeQuick(Patch in_Patch, ISeed in_Seed)
        {
            ...
            in_Patch.Add(0x02C86E, in_Seed.GetNextUInt8(1, 11), "Quickman Number of Boomerangs");

            //0x02C882 - Boomerang: delay before arc 37. 0 for no arc, or above like 53. do from 5 to 53.
            in_Patch.Add(0x02C882, in_Seed.GetNextUInt8(5, 54), "Quickman Boomerang Delay 1");

            //0x02C887 - Boomerang speed when appearing, 4, do from 1 to 7
            in_Patch.Add(0x02C887, in_Seed.GetNextUInt8(1, 8), "Quickman Boomerang Velocity Integer 1");

            //0x03B726 - Boomerang speed secondary, 4, does this affect anything else?
            in_Patch.Add(0x03B726, in_Seed.GetNextUInt8(1, 8), "Quickman Boomerang Velocity Integer 2");

            // For all jumps, choose randomly from 2 to 10
            //0x02C8A3 - Middle jump, 7
            in_Patch.Add(0x02C8A3, in_Seed.GetNextUInt8(2, 11), "Quickman Jump Height 1 Integer");

            //0x02C8A4 - High jump, 8
            in_Patch.Add(0x02C8A4, in_Seed.GetNextUInt8(2, 11), "Quickman Jump Height 2 Integer");

            //0x02C8A5 - Low jump, 4
            in_Patch.Add(0x02C8A5, in_Seed.GetNextUInt8(2, 11), "Quickman Jump Height 3 Integer");

            //0x02C8E4 - Running time, 62, do from 24 to 80
            in_Patch.Add(0x02C8E4, in_Seed.GetNextUInt8(24, 81), "Quickman Running Time");

            //0x02C8DF - Running speed, 2, do from 1 to 5
            in_Patch.Add(0x02C8DF, in_Seed.GetNextUInt8(1, 6), "Quickman Running Velocity Integer");
        }
        ...
        protected void ChangeFlash(Patch in_Patch, ISeed in_Seed)
        {
            ...
            in_Patch.Add(0x02C982, in_Seed.GetNextUInt8(4), "Flashman Walk Velocity Integer");

            //0x02C97D - Walk velocity fraction 6, do 0 to 255
            in_Patch.Add(0x02C97D, in_Seed.GetNextUInt8(256), "Flashman Walk Velocity Fraction");

            //0x02C98B - Delay before time stopper 187 frames. Do from 30 frames to 240 frames
            in_Patch.Add(0x02C98B, in_Seed.GetNextUInt8(30, 241), "Flashman Delay Before Time Stopper");

            //0x02CAC6 - Jump distance integer 0, do 0 to 3
            // TODO do fraction also
            in_Patch.Add(0x02CAC6, in_Seed.GetNextUInt8(4), "Flashman Jump X-Velocity Integer");

            //0x02CACE - Jump height 4, do 3 to 8
            in_Patch.Add(0x02CACE, in_Seed.GetNextUInt8(3, 9), "Flashman Jump Y-Velocity Integer");

            //0x02CA81 - Projectile speed 8, do 2 to 10
            in_Patch.Add(0x02CA81, in_Seed.GetNextUInt8(2, 11), "Flashman Projectile Velocity Integer");

            //0x02CA09 - Number of projectiles to shoot 6, do 3 to 16
            in_Patch.Add(0x02CA09, in_Seed.GetNextUInt8(3, 17), "Flashman Number of Projectiles");
        }
        ...
        protected void ChangeMetal(Patch in_Patch, ISeed in_Seed)
        {
            ...
            in_Patch.Add(0x02CC3F, in_Seed.GetNextUInt8(2, 10), "Metalman Projectile Velocity Integer");

            //0x02CC1D - Odd change to attack behavior, 0x06, only if different than 6. Give 25% chance.
            if (in_Seed.GetNextDouble() > 0.75)
            {
        ...
        protected void ChangeCrash(Patch in_Patch, ISeed in_Seed)
        {
            ...
            in_Patch.Add(0x02CCf2, in_Seed.GetNextUInt8(), "Clashman Walk X-Velocity Fraction");

            //0x02CCF7 - Walk x-vel integer 0x01, do 0 to 2
            in_Patch.Add(0x02CCF7, in_Seed.GetNextUInt8(2), "Clashman Walk X-Velocity Integer");
            ...
            in_Patch.Add(0x02CD2A, in_Seed.GetNextUInt8(2, 11), "Clashman Jump Y-Velocity Integer");

            //0x02CDD3 - Shot behavior, 0x5E, change to have him always shoot when jumping, 20% chance
            if (in_Seed.GetNextDouble() > 0.80)
            {
                in_Patch.Add(0x02CDD3, 0x50, "Clashman Disable Single Shot");
            }

            //0x02CDEE - Crash Bomber velocity, 0x06, do from 2 to 8
            in_Patch.Add(0x02CDEE, in_Seed.GetNextUInt8(2, 9), "Clashman Clash Bomber X-Velocity");
        }
    }
}
